...
document.addEventListener('change', async (e) => {
  ...
  try {
    ...
    if (el.matches('#mobile-report input[type=file]')) {
      ...
      if (!file) return;
      // Immediately show preview using ObjectURL (no size limit issue)
      const previewUrl = URL.createObjectURL(file);
      const previewContainer = document.querySelector('.m-photo-preview');
      if (previewContainer) {
        previewContainer.innerHTML = `<figure class="photo-evidence"><img class="evidence" src="${previewUrl}" alt="Foto unggahan ${mobile().draft.category || ''}" loading="lazy"><span>BUKTI UNGGAHAN</span></figure>`;
        // Update upload button text
        const uploadLabel = document.querySelector('.m-upload');
        if (uploadLabel) {
          const svgIcon = uploadLabel.querySelector('svg');
          const svgHtml = svgIcon ? svgIcon.outerHTML : '';
          uploadLabel.innerHTML = `${svgHtml} Ganti foto<input type="file" name="photo" accept="image/png,image/jpeg,image/webp" capture="environment">`;
        }
      }
      // Read data URL and save to draft
      try {
        const photo = await ctx.readPhoto(file);
        if (photo) {
          mobile().draft.photo = photo;
          persist();
        }
      } catch (readErr) {
        // Even if readPhoto fails (e.g. file too big), keep the preview visible
        // but store the ObjectURL as fallback
        mobile().draft.photo = previewUrl;
        persist();
        say(readErr.message);
      }
    }
